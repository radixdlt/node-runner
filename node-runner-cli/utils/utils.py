...
def print_vote_and_fork_info(health, engine_configuration):    
    ...
    if health['current_fork_name'] == newest_fork_name:
        ...
        print(f"{bcolors.WARNING}No vote is required{bcolors.ENDC}")
        return
    ...
    if not is_candidate:
        ...
        print(f"{bcolors.WARNING}This newer fork is not a candidate fork, so no vote is required{bcolors.ENDC}")
        return
    ...
    if not node_says_vote_required:
        ...
        print(f"{bcolors.WARNING}The node has already voted for this candidate fork, so no new vote is required{bcolors.ENDC}")
        return

    print(f"The node is currently running fork {bcolors.BOLD}{health['current_fork_name']}{bcolors.ENDC}. The node is aware of a newer candidate fork ({newest_fork_name})")
    print(f"{bcolors.WARNING}The node has not yet cast a vote for this fork{bcolors.ENDC}")
